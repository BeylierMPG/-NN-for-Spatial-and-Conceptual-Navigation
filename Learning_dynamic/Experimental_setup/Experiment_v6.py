...
class Experiment:
    ...
    def Generate_data(self):
        ...
        if self.size_output_multi == 2:
            target_p = [[int(np.abs(a[1] - a[0]) <= self.width_ones/self.width),0] if a[5] == 0 else [0,int(np.abs(a[2] - a[3]) <= self.width_ones/self.width and np.abs(a[3] - a[4]) <= self.width_ones/self.width and np.abs(a[2] - a[4]) <= self.width_ones/self.width)] for a in data_liste] 
            target_n = [[int(np.abs(a[1] + a[0] - 1) <= self.width_ones/self.width),0] if a[5] == 0 else [0,int(np.abs(a[2] + a[3] - 1) <= self.width_ones/self.width and np.abs(a[3] + a[4] - 1) <= self.width_ones/self.width and np.abs(a[2] + a[4] - 1) <= self.width_ones/self.width)] for a in data_liste] 
        ...
        target = torch.tensor(target_p,dtype = torch.float32)
        
        
        dataset_multi =  TensorDataset(data[0:len(data_space)],target[0:len(data_space)])
        dataset_space = TensorDataset(data_space,target_space)
        dataset_weather =  TensorDataset(data_weather,target_weather)

        print("Size multi:",len(dataset_multi))
        print("Size space:",len(data_space))
        print("Size weather:",len(data_weather))

        return dataset_multi,dataset_space,dataset_weather
        # a = 0.7
        # b = 0.9
